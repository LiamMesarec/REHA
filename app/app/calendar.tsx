...
export const getMonthEvents = (
  ...
  for (let i = 1; i <= getDaysInMonth(year, month); i++) {
    ...
    for (let j = 0; j < events2.length; j++) {
      // Could remove them after used and get them for each month from db
      let event = events2[j];
    ...
    >
      ...
      <FlatList
          ...
          if (item.header === false) {
            if (item.event !== "No event") {
              return (
                <TouchableOpacity
                  onPress={() =>
                    router.push(`/event_detailed?eventId=${item.id}`)
                  }
                >
                  <DayEvent {...item} />
                </TouchableOpacity>
        ...
        onEndReached={() => {
          ...
          const newEvents = getMonthEvents(
            dateDisplayed.getMonth(),
            dateDisplayed.getFullYear(),
            events2
          );
